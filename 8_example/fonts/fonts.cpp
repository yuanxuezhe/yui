...
void draw(fl_Widget w){
  ...
  instance_data* d = (instance_data*)Fl_Widget_other_data(w);
...
void font_cb(fl_Widget w, void* user_data) {
  ...
  if (!fn) return;
  fn--;
  instance_data* d = (instance_data*)Fl_Widget_other_data(textobj);
  d->font = fn;
...
int main (int argc, char** argv){
  ...
  numsizes = (int *)malloc(sizeof(int) * k);
  i = 0;
  for (; i < k; i++) {
    int t;
    ...
    numsizes[i] = n;
    if (n) {
      sizes[i] = (int *)malloc(sizeof(int) * n);
      int j = 0;
