...
#include <yscore/Fl_Table_RowC.h>
// #include <Fl_DrawC.h>
...
Row* to_row(string* arr, int length){
  int* length_cpy;
  length_cpy = (int*)malloc(sizeof(int));
  *length_cpy = length;
...
string* extract_index(Row* arr[], int rows, int index){
  int i = 0;
  string* result = (string*)malloc(sizeof(string) * rows);
  for(;i<rows;i++){
...
Row** to_array(Node* start){
  ...
  Row** into;
  into = (Row**)malloc(sizeof(Row) * (*count));
  int soFar = 0;
  accumulator* accum = (accumulator*)malloc(sizeof(accumulator));
  accum->soFar = &soFar;
  accum->row = into;
...
void initializeTable(fl_Table_Row table){
  other_data* d = (other_data*)malloc(sizeof(other_data));
  d->_sort_reverse = (int*)malloc(sizeof(int));;
  *(d->_sort_reverse) = 0;
  d->_sort_lastcol = (int*)malloc(sizeof(int));
  *(d->_sort_lastcol) = -1;
  d->_rowData = (Node*)malloc(sizeof(Node));
  d->_rowData = NULL;
...
void load_command(fl_Table_Row table, const string cmd){
  ...
  for (;fgets(s, sizeof(s)-1, fp); r++){
    ...
    for(;(t==0)?(ss=strtok(s,delim)):(ss=strtok(NULL, delim));t++) {
      ...
      *length = 1;
      string* contents = (string*)malloc(sizeof(char*));
...
void test(){
  ...
  int sort_index = 0;
  Row** rows;
  rows = (Row**)malloc(sizeof(Row*) * arr_length);
  rows[0] = a_row;
  rows[1] = b_row;
  rows[2] = c_row;
  rows[3] = d_row;
